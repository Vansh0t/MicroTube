...
namespace MicroTube.Services.VideoContent.Processing
{
	public class DefaultVideoProcessingPipeline : IVideoProcessingPipeline
	{
		...
		public async Task<IServiceResult> Process(string videoFilePath, CancellationToken cancellationToken)
		{
			...
			try
			{
				...
				await _videoDataAccess.UpdateUploadStatus(uploadProgress.Id.ToString(), VideoUploadStatus.InProgress);
				cancellationToken.ThrowIfCancellationRequested();
				...
				cancellationToken.ThrowIfCancellationRequested();
				await _videoDataAccess.UpdateUploadStatus(uploadProgress.Id.ToString(), VideoUploadStatus.Success);
				fileStream.Dispose();
			...
			catch(TaskCanceledException)
			{
				...
				if (uploadProgress != null)
					await _videoDataAccess.UpdateUploadStatus(uploadProgress.Id.ToString(), VideoUploadStatus.Fail);
				return ServiceResult.FailInternal();
			}
			catch(Exception e)
			{
